...
class Database {
    ...
    async createTables() {
        const tables = [
            ...
            `CREATE TABLE IF NOT EXISTS crm_orders (
                ...
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (client_id) REFERENCES crm_leads (id) ON DELETE SET NULL
            )`,

            // Blog Posts & Security Advisories table
    ...
    async seedDefaultData() {
        ...
        if (existingAdmin.count === 0) {
            ...
            const usersToSeed = [
                { username: 'admin', email: 'spectraopsofficial@gmail.com', name: 'Rana Talha Majid', role: 'super_admin' },
                { username: 'support', email: 'support@spectraops.pk', name: 'Security Support Specialist', role: 'support' },
                { username: 'hr', email: 'hr@spectraops.pk', name: 'Hiring Executive', role: 'hr' },
                { username: 'finance', email: 'billing@spectraops.pk', name: 'Finance Controller', role: 'finance' },
                { username: 'content', email: 'content@spectraops.pk', name: 'Content Editor', role: 'content_manager' }
            ];
            ...
            for (const u of usersToSeed) {
                await this.run(`
                    ...
                    VALUES (?, ?, ?, ?, ?)
                `, [u.username, u.email, hashedPassword, u.name, u.role]);
            }

            console.log('✅ Default users with RBAC roles seeded');
        }
    }
